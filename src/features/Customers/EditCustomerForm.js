...
import { customersSchema } from '../../assets/schema/schema'

const EditCustomerForm = ({ customer, onSave, onDelete, onCancel  }) => {
  const {
    register,
    handleSubmit,
    formState: { errors }
    ...
    othername: customer?.name?.othername,
    gender: customer?.gender,
    phone: customer?.contact?.phone,
    address: customer?.contact?.address
  }, resolver: yupResolver(customersSchema), mode: 'onBlur'})
        ...
        <div className='w-full px-6 py-6 bg-white dark:text-gray-800
            ...
            <form onSubmit={handleSubmit(onSave)}>
              <div className='flex justify-between items-center border-b border-gray-200 mb-4'>
                <h2 className='text-xl font-bold text-center pb-2 '>Update Customer's Records</h2>
